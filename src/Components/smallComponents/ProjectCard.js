...
const ProjectCard = ({Title, Tags, Description, Link, Imagelink}) => {
    return (
        <a href={Link} target='_blank' rel="noreferrer" className="xl:w-1/4 md:w-1/2 w-full md:pr-2 pb-2 ">
            <div className="bg-black bg-opacity-40 hover:bg-opacity-100 duration-300 p-1 rounded-lg h-full">
                {window.innerWidth < 768 && <div className="text-lg hover:text-blue-700 text-green-100 font-medium title-font mb-4">{Title}</div>}
